    ...
    class Program
    {
        ...
        static async Task ProcessMessagesAsync(Message message, CancellationToken token)
        {
           ...
           // Console.WriteLine($"Received message: SequenceNumber:{message.SystemProperties.SequenceNumber} Body:{Encoding.UTF8.GetString(message.Body)}");
            ...
            {
                { "LicensePlateCaptureTime", Convert.ToString(values["LicensePlateCaptureTime"])},
                { "LicensePlate", Convert.ToString(values["LicensePlate"])},
                { "Latitude", Convert.ToString(values["Latitude"])},
                { "Longitude", Convert.ToString(values["Longitude"])},
            };
